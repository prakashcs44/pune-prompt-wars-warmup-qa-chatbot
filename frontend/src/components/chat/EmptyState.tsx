import { GraduationCap, BookOpen, Brain, Activity, Target, Sparkles } from 'lucide-react';
...
export function EmptyState({ onAction, suggestedPrompts }: EmptyStateProps) {
    ...
    <div className="flex flex-col items-center justify-center h-full gap-6 select-none px-4">
      ...
      <div className="text-center max-w-md">
        ...
        <p className="text-sm text-slate-400 leading-relaxed font-medium">
          {hasSuggestions 
            ? "Based on your interests and progress, here are some personalized suggestions to get you started:"
            : "I'm your personalized AI tutor. I adapt to your pace, remember your history, and help you bridge knowledge gaps across sessions."}
        </p>
      </div>
          ...
          suggestedPrompts.map((prompt) => (
            ...
            >
              <div className="w-10 h-10 rounded-xl bg-slate-800 flex items-center justify-center group-hover:bg-primary-500/10 transition-colors">
                <Sparkles className="w-5 h-5 text-primary-400 group-hover:text-primary-300" />
              </div>
              <div className="flex-1">
                <p className="text-sm text-slate-200 font-bold leading-tight">{prompt}</p>
              </div>
            </div>
          ))
        ) : (
          defaultPrompts.map(({ icon: Icon, label, desc }) => (
            <div
              key={label}
